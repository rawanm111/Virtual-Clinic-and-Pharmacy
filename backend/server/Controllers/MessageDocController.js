...
const createMessage = async (req, res) => {
    ...
    try {
        const existingChat = await Message.findOne({ participantDoc: sender, participantPatient: receiver });   
        console.log(existingChat);

        if (existingChat) 
        await Message.updateOne(
            { participantDoc: sender, participantPatient: receiver },
            {
                $push: {
                    'aggregatedMessages': {
                        sender,
            ...
            const newChat = new Message({
                participantDoc: sender,
                participantPatient: receiver,
                aggregatedMessages: [{
                    sender,
                    receiver,
                    text,
