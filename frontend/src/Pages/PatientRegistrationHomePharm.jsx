    ...
    setTimeout(() => {
      ...
      .catch((error) => {
        if (error.response) {
          ...
          setNotification({ type: 'error', message: 'No response received from the server.' });
        } else {
          console.error('Error setting up the request:', error.message);
          setNotification({ type: 'error', message: 'Error setting up the request.' });
        }
      });
    ...
    <TextField
        ...
        <Box style={{ padding: '20px', marginBottom: '20px',backgroundColor: 'white', borderRadius: '8px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)', width: '70%' }}>
          ...
          <TextField
                ...
                error={Boolean(errors.emergencyContactFullNameNumber)}
